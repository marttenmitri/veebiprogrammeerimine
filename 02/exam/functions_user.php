...
function signUp($name, $surname, $email, $gender, $birthDate, $password){
	...
	if($stmt->execute()){
	  ...
	  if($stmt->fetch()){
		...
		if(password_verify($password, $passwordFromDb)){
		  ...
		  $stmt->close();
		  $stmt = $conn->prepare("SELECT bgcolor, txtcolor, picture FROM vpuserprofiles1 WHERE userid=?");
		  echo $conn->error;
		  $stmt->bind_param("i", $_SESSION["userId"]);
		  $stmt->bind_result($bgColorFromDb, $txtColorFromDb, $picture);
		  $stmt->execute();
		  if($stmt->fetch()){
			...
			$_SESSION["picture"] = "../photos/vp_user_generic_tiny.png";
  ...
  function myPic($picture){
	...
	if($stmt->fetch()){
		//profiil juba olemas, uuendame
		$stmt->close();
		$stmt = $conn->prepare("UPDATE vpuserprofiles1 SET picture = ? WHERE userid=?");
		echo $conn->error;
		...
		//profiili pole, salvestame
		$stmt->close();
		$stmt = $conn->prepare("INSERT INTO vpuserprofiles1 (userid, picture) VALUES(?,?)");
		echo $conn->error;
		$stmt->bind_param("is", $_SESSION["userId"], $picture);
		if($stmt->execute()){
			$_SESSION["picture"] = $picture;
			$notice = "Profiil edukalt salvestatud!";
		} else {
